...
from .serializer import (RegistrationSerializer, CustomAuthTokenSerializer, 
                         CustomTokenObtainPairSerializer, ChangePaswordSerializer, 
...
class TestEmailSend(generics.GenericAPIView):
    def post(self, request):
        self.email = 'ho@gmail.com'
        user_obj = get_object_or_404(User, email=self.email)
        token = self.get_tokens_for_user(user_obj)
        email_obj = EmailMessage('email/activation_email.tpl', {'token': token}, 'ho@gmail.com',to=[self.email])
        EmailThread(email_obj).start()
